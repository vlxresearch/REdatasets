...
- Data Format:
Relation Extraction datasets store tokenized sentences (token) with annotated pairs of entities (t, h) and the relation between them (relation).
For each entity we have its position in the text (pos), its Freebase identifier (mid), the entity type (type), the surface form of the entity in the text (name). We also store the origin public dataset for each sentence (origin_dataset).
